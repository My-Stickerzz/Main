const PRICING = {
  TIER1: { max: 39, price: 0.75 },
  TIER2: { max: 80, price: 0.6 },
  TIER3: { price: 0.5 },
};
...
const products = [
  {
    ...
    image: "IMG-0791.JPG",
    intro:
      "Van Gogh Pack <br> pack with 18 different high quality stickers in the theme of Van Gogh <br> Product with great reviews",
    category: "pack",
    theme: "Van Gogh",
  },
  {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  productList.innerHTML =
    '<div class="text-center"><i class="fas fa-spinner fa-spin fa-3x text-warning"></i></div>';
...
function displayPagination(totalProducts) {
  ...
  paginationContainer.innerHTML = `
        <a href="#display"><button class="btn btn-warning mx-1" onclick="changePage(${
          currentPage - 1
        })" ${currentPage === 1 ? "disabled" : ""}>
            <b><=</b>
        </button>
        <span class="mx-3 pt-2">Page ${currentPage} of ${totalPages}</span>
        <button class="btn btn-warning mx-1" onclick="changePage(${
          currentPage + 1
...
function validatePurchase() {
  ...
  const hasPack = shoppingCart.some(
    (item) => item.category?.toLowerCase() === "pack"
  );
...
function addToCart(productName, price, quantity) {
  ...
  Swal.fire({
    ...
    html: `Added ${quantity} to Cart.<br>Price per unit: ${(
...
function addFreeSticker() {
  ...
  const randomSticker =
    stickerProducts[Math.floor(Math.random() * stickerProducts.length)];
  shoppingCart.push({
...
function checkout() {
  ...
  const totalPrice = shoppingCart.reduce(
    (total, item) => total + (item.isFree ? 0 : item.price * item.count),
    0
  );
